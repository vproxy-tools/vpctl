...
import (
	...
	client "github.com/vproxy-tools/vpctl/swagger/vproxy_client"
...
func getServerGroupInUpstream(ups *Upstream, sample *ServerGroupInUpstream) (int, *ServerGroupInUpstream) {
	for idx, x := range ups.Spec.ServerGroups {
...
func buildServerGroupUpdateList(client *client.Vproxy, now *Upstream, old *Upstream, ret []*Todo) []*Todo {
	...
	for _, oldSg := range old.Spec.ServerGroups {
		if _, tmp := getServerGroupInUpstream(now, &oldSg); tmp == nil {
	...
	for _, nowSg := range now.Spec.ServerGroups {
		...
		idx, oldSg := getServerGroupInUpstream(old, &nowSg)
		x := nowSg
		if oldSg == nil {
			// should create
			ret = append(ret, &Todo{
...
func getStaticServerInServerGroup(sg *ServerGroup, sample *StaticServer) (int, *StaticServer) {
	for idx, x := range sg.Spec.Servers.Static {
...
func buildStaticServerInServerGroupUpdateList(client *client.Vproxy, now *ServerGroup, old *ServerGroup, ret []*Todo) []*Todo {
	...
	for _, oldSvr := range old.Spec.Servers.Static {
		if _, tmp := getStaticServerInServerGroup(now, &oldSvr); tmp == nil {
	...
	for _, nowSvr := range now.Spec.Servers.Static {
		...
		idx, oldSvr := getStaticServerInServerGroup(old, &nowSvr)
		x := nowSvr
		if oldSvr == nil {
			// should create
			ret = append(ret, &Todo{
...
func getSecurityGroupRuleInSecurityGroup(secg *SecurityGroup, sample *SecurityGroupRule) (int, *SecurityGroupRule) {
	for idx, x := range secg.Spec.Rules {
...
func buildSecurityGroupRuleInSecurityGroupUpdateList(client *client.Vproxy, now *SecurityGroup, old *SecurityGroup, ret []*Todo) []*Todo {
	...
	for _, oldRule := range old.Spec.Rules {
		if _, tmp := getSecurityGroupRuleInSecurityGroup(now, &oldRule); tmp == nil {
	...
	for _, nowRule := range now.Spec.Rules {
		...
		idx, oldRule := getSecurityGroupRuleInSecurityGroup(old, &nowRule)
		x := nowRule
		if oldRule == nil {
			// should create
			ret = append(ret, &Todo{
