...
docker_dev_vpctl:
	...
	docker build --no-cache -t vpctl:local ./misc/dockerfiles/vpctl

docker_dev_vproxy:
	docker rmi -f vproxy:local
	cp ../vproxy/build/libs/vproxy.jar ./misc/dockerfiles/vproxy/vproxy.jar
	docker build --no-cache -t vproxy:local ./misc/dockerfiles/vproxy
