...
// the DnsServer object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.14.1/pkg/reconcile
func (r *DnsServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
...
func (r *DnsServerReconciler) reconcile(ctx context.Context, logger logr.Logger, o *m.DnsServer) (res ctrl.Result, err error) {
	var todo []*c.Todo

	if o.DeletionTimestamp != nil {
		// need to be deleted
		todo, err = c.DeleteByConfig([]c.Config{
			&c.DnsServer{Base: formatResourceBase(o.ObjectMeta)},
		})
	} else {
		...
		o.Spec.DnsServerSpec.DeepCopyInto(&apply.Spec)
		addNsToResourceName(o.Namespace, &apply.Spec.RRSets)
